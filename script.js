(function () {
  ...
  `;
    ...
    function openBkMod() {
      ...
      const t = curT, fee = calcFee(t.price), tot = +(Number(t.price || 0) + fee).toFixed(2);
    ...
    async function confirmBk() {
      ...
      const fee = calcFee(t.price), tot = +(Number(t.price || 0) + fee).toFixed(2);
      try {
    ...
    async function loadTxList() {
      ...
      try {
        ...
        if (isTutor && typeof loadWdHistory === 'function') await loadWdHistory().catch(() => {});
        ...
        console.warn('wallet load failed', e?.message || e);
        el.innerHTML = '<div class="empty" style="padding:40px"><div class="emptyic">⚠️</div><p>تعذر تحميل المحفظة حالياً</p><p style="font-size:.8rem;color:var(--muted);margin-top:6px">أعد المحاولة بعد قليل</p></div>';
      }
    }
...
(function(){
          ...
          <div class="skillak-crop-shell">
            <div class="skillak-crop-canvas-wrap">
              <div class="skillak-crop-head">
                <div>
                  ...
                  <div class="skillak-crop-sub">حرّك الصورة أو كبّرها حتى تصل للشكل المناسب للبروفايل، ثم احفظ النسخة النهائية فقط.</div>
                </div>
            ...
            <div class="skillak-crop-side">
              <div class="skillak-crop-preview-wrap">
                ...
                <div class="skillak-crop-preview"><canvas id="skillakCropPreview" width="280" height="280"></canvas><canvas id="skillakCropOutput" width="840" height="840" style="display:none"></canvas></div>
              </div>
  ...
  function drawSkillakCrop(canvas, preview = false, showGuides = true) {
  ...
  function renderSkillakCrop() {
    ...
    drawSkillakCrop(canvas, false, true);
    drawSkillakCrop(preview, true, false);
  }

  async function applySkillakCrop() {
    const canvas = document.getElementById('skillakCropCanvas');
    if (!canvas || !cropState.image) return;
    const outCanvas = document.getElementById('skillakCropOutput') || canvas;
    drawSkillakCrop(outCanvas, false, false);
    const out = outCanvas.toDataURL('image/jpeg', 0.96);
  ...
  async function loadPlatformSettings() {
    try {
      ...
      if (s.exists) {
        ...
        const feeLabel = document.getElementById('bkFeeLabel');
        if (feeLabel) feeLabel.textContent = `📊 رسوم الخدمة (${platformCommissionRate}%)`;
      }
    } catch (e) {
  ...
  async function doGoogleLogin(mode = 'login') {
    if (!isGoogleAuthSupported()) {
      showT('تسجيل Google لا يعمل من ملف محلي. شغّل الموقع عبر https أو localhost (Firebase Hosting / Live Server) ثم أعد المحاولة.', 'err');
      return;
    }
  ...
  window.loadTxList = async function() {
    ...
    try {
      ...
      if (el) el.innerHTML = '<div class="empty" style="padding:40px"><div class="emptyic">⚠️</div><p>تعذر تحميل المحفظة حالياً</p><p style="font-size:.8rem;color:var(--muted);margin-top:6px">أعد المحاولة بعد قليل</p></div>';
      console.warn('wallet load failed', e?.message || e);
      return undefined;
    }
  };
  ...
  async function renderReportsTab(container) {
    ...
    const totals = users.reduce((a, u) => {
      a.total++;
      if (u.role === 'learner') a.learners++;
      else if (u.role === 'tutor') a.tutors++;
      else if (u.role === 'both') a.both++;
      else if (u.role === 'admin') a.admins++;
      return a;
    }, { total: 0, learners: 0, tutors: 0, both: 0, admins: 0 });

    container.innerHTML = `
      <div class="card">
        <div class="ch">
          <div class="ct">📑 التقارير الشاملة</div>
          <div class="fh">واجهة عربية مرتبة، مع ملف PDF باسم صاحب التقرير</div>
        </div>
        <div class="cb" style="overflow:auto">
          <div class="srow" style="margin-bottom:16px">
            <div class="sc"><div class="scic">👥</div><div class="scval">${totals.total}</div><div class="sclbl">كل المستخدمين</div></div>
            <div class="sc"><div class="scic">🎓</div><div class="scval">${totals.tutors}</div><div class="sclbl">المعلمين</div></div>
            <div class="sc"><div class="scic">📚</div><div class="scval">${totals.learners}</div><div class="sclbl">المتعلمين</div></div>
            <div class="sc"><div class="scic">⚖️</div><div class="scval">${platformCommissionRate}%</div><div class="sclbl">عمولة المنصة</div></div>
          </div>
          <table class="skillak-report-table">
            <thead>
              <tr>
                <th>الاسم</th><th>البريد</th><th>الدور</th><th>الدولة</th><th>الرصيد</th><th>الأرباح</th><th>شحن</th><th>سحب</th><th>الجلسات</th><th>PDF</th><th>حذف</th>
              </tr>
            </thead>
            <tbody>
              ${await Promise.all(users.map(async (u) => {
                ...
                const canTopup = role !== 'tutor';
                const canWithdraw = role !== 'learner';
                const [w, stu, tut, pay, wd] = await Promise.all([
                  db.collection('wallets').doc(u.id).get().catch(() => null),
                  db.collection('bookings').where('studentId', '==', u.id).get().catch(() => ({ docs: [] })),
                  db.collection('bookings').where('tutorId', '==', u.id).get().catch(() => ({ docs: [] })),
                  db.collection('paymentRequests').where('userId', '==', u.id).get().catch(() => ({ docs: [] })),
                  db.collection('withdrawalRequests').where('userId', '==', u.id).get().catch(() => ({ docs: [] }))
                ]);
                const balance = w?.exists ? Number(w.data().balance || 0) : 0;
                const allBk = [...stu.docs, ...tut.docs].map(d => d.data());
                const completed = allBk.filter(b => b.status === 'completed').length;
                const earnings = role === 'learner' ? 0 : allBk.reduce((s, b) => s + Number((b.price || 0) - (b.fee || 0)), 0);
                const topup = pay.docs.map(d => d.data()).filter(x => x.status === 'approved').reduce((s, x) => s + Number(x.amount || 0), 0);
                const withdraw = wd.docs.map(d => d.data()).filter(x => x.status === 'approved').reduce((s, x) => s + Number(x.amount || 0), 0);
                return `<tr>
                  <td style="max-width:180px;word-break:break-word;overflow-wrap:anywhere">${escapeHTML(u.name || '—')}</td>
                  <td title="${escapeHTML(u.email || '')}" style="max-width:220px;word-break:break-word;overflow-wrap:anywhere">${escapeHTML(u.email || '—')}</td>
                  <td><span class="tag ${role === 'tutor' ? 'tag-g' : role === 'admin' ? 'tag-r' : ''}">${roleLabel(role)}</span></td>
                  <td style="max-width:140px;word-break:break-word;overflow-wrap:anywhere">${escapeHTML(u.country || '—')}</td>
                  <td>${balance.toFixed(2)} ج.م</td>
                  <td>${canWithdraw ? earnings.toFixed(2) + ' ج.م' : '—'}</td>
                  <td>${canTopup ? topup.toFixed(2) + ' ج.م' : '—'}</td>
                  <td>${canWithdraw ? withdraw.toFixed(2) + ' ج.م' : '—'}</td>
                  <td>${completed}</td>
                  <td><button class="btn btn-xs" style="background:var(--teal);color:#fff" onclick="downloadUserReportPdf('${u.id}')">PDF</button></td>
                  <td><button class="btn btn-d btn-xs" onclick="delU('${u.id}',this)">🗑️</button></td>
                </tr>`;
              })).join('')}
            </tbody>
          </table>
  ...
  window.savePlatformCommission = async function() {
    ...
    const feeLabel = document.getElementById('bkFeeLabel');
    if (feeLabel) feeLabel.textContent = `📊 رسوم الخدمة (${val}%)`;
