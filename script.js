(function () {
  ...
  `;
    ...
    async function loadTxList() {
      ...
      const showTopup = role !== 'tutor';
      const showWithdraw = role !== 'learner';
      const showSpend = role !== 'tutor';
      const showEarn = role !== 'learner';
      const wCard = document.getElementById('withdrawCard');
      const topCard = document.getElementById('topupCard');
      if (wCard) wCard.style.display = showWithdraw ? 'block' : 'none';
      if (topCard) topCard.style.display = showTopup ? 'block' : 'none';
      if (wCard && showWithdraw) loadWdHistory();
      const wInLbl = document.getElementById('wInLbl');
      const wOutLbl = document.getElementById('wOutLbl');
      if (role === 'learner') {
        if (wInLbl) wInLbl.textContent = 'إجمالي الشحن';
        if (wOutLbl) wOutLbl.textContent = 'إجمالي الإنفاق';
      } else if (role === 'tutor') {
        if (wInLbl) wInLbl.textContent = 'إجمالي الأرباح';
        if (wOutLbl) wOutLbl.textContent = 'إجمالي السحب';
      } else {
        if (wInLbl) wInLbl.textContent = 'إجمالي الداخل';
        if (wOutLbl) wOutLbl.textContent = 'إجمالي الخارج';
      }
      ...
      const visible = [...snap.docs].map(d => ({ id: d.id, ...d.data() }))
        .filter(tx => {
          ...
          if (kind === 'topup') return showTopup;
          if (kind === 'withdrawal') return showWithdraw;
          if (kind === 'booking') return String(tx.type || '').toLowerCase() === 'debit' ? showSpend : showEarn;
          if (!kind) {
            const desc = String(tx.description || '').toLowerCase();
            return /شحن|سحب|withdraw|top.?up|payment|حجز|جلسة|أرباح/.test(desc);
          }
          return false;
        })
        .sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
      ...
      visible.forEach(tx => {
        ...
        if (kind === 'booking' && type === 'credit') totalIn += tx.amount || 0;
        if (kind === 'booking' && type === 'debit') totalOut += tx.amount || 0;
      });
      ...
      const statusPill = (tx, kind) => {
        const status = String(tx.status || '').toLowerCase();
        if (kind === 'booking') {
          return String(tx.type || '').toLowerCase() === 'credit'
            ? '<span class="pill pc">✅ أرباح</span>'
            : '<span class="pill pp">⏳ محجوز</span>';
        }
        const map = {
          ...
          held: '<span class="pill pp">⏳ محجوز</span>'
        };
      ...
      el.innerHTML = visible.map(tx => {
        ...
        const desc = tx.description || (kind === 'booking' ? (String(tx.type || '').toLowerCase() === 'credit' ? 'أرباح جلسة' : 'حجز جلسة') : isIn ? 'شحن محفظة' : isOut ? 'سحب أرباح' : '-');
        const amtSign = isIn ? '+' : '-';
        ...
        try {
          ...
          setTimeout(() => {
            if (bk?.tutorId === CU?.uid) {
              ...
              openM('revMod');
              showT('✅ تم إنهاء الجلسة. سيُضاف الرصيد فقط بعد اعتماد الإدارة.', 'suc');
            } else {
  ...
  <div class="dsec">${revs.length ? revs.map(r => `<div style="padding:15px 18px;border-bottom:1px solid var(--border)"><div style="display:flex;justify-content:space-between;margin-bottom:6px"><div style="font-weight:600">${r[isTutor ? 'studentName' : 'tutorName'] || '—'} <span class="stars">${'★'.repeat(r.rating || 5)}</span></div><div style="font-size:.71rem;color:var(--muted)">${r.createdAt?.toDate ? r.createdAt.toDate().toLocaleDateString('ar-SA') : ''}</div></div><p style="font-size:.83rem;color:#374151">${r.comment || ''}</p></div>`).join('') : '<div style="text-align:center;padding:32px;color:var(--muted)">لا توجد تقييمات بعد</div>'}</div>`;
    ...
    function gRS(step) {
      if (step === 2) { setTimeout(initCountrySelectors, 20); }
      if (step === 3) {
    ...
    async function doReg() {
      ...
      try {
        ...
        const profile = {
          ...
          lang: 'عربي', country: document.getElementById('r2Country')?.value || '', phoneCode: document.getElementById('r2Code')?.value || '', category: '', rating: 0,
        ...
        if (isTutor) {
          ...
          profile.phoneCode = document.getElementById('r2Code')?.value || '';
    ...
    async function deleteDocByQuery(query) {
      const snap = await query.get().catch(() => null);
      if (!snap || snap.empty) return 0;
      let count = 0;
      for (const doc of snap.docs) {
        try { await doc.ref.delete(); count += 1; } catch (e) {}
      }
      return count;
    }

    async function deleteUserData(uid, { deleteAuth = false } = {}) {
      if (!uid) return;
      const sessionDocs = [];
      const [s1, s2] = await Promise.all([
        db.collection('sessions').where('studentId', '==', uid).get().catch(() => ({ docs: [] })),
        db.collection('sessions').where('tutorId', '==', uid).get().catch(() => ({ docs: [] }))
      ]);
      [...(s1.docs || []), ...(s2.docs || [])].forEach(d => { if (!sessionDocs.find(x => x.id === d.id)) sessionDocs.push(d); });
      for (const doc of sessionDocs) {
        const chatSnap = await doc.ref.collection('chat').get().catch(() => ({ docs: [] }));
        for (const c of chatSnap.docs || []) await c.ref.delete().catch(() => {});
      ...
      await deleteDocByQuery(db.collection('bookings').where('studentId', '==', uid));
      await deleteDocByQuery(db.collection('bookings').where('tutorId', '==', uid));
      await deleteDocByQuery(db.collection('reviews').where('studentId', '==', uid));
      await deleteDocByQuery(db.collection('reviews').where('tutorId', '==', uid));
      await deleteDocByQuery(db.collection('messages').where('senderId', '==', uid));
      await deleteDocByQuery(db.collection('messages').where('receiverId', '==', uid));
      await deleteDocByQuery(db.collection('transactions').where('userId', '==', uid));
      await deleteDocByQuery(db.collection('withdrawalRequests').where('userId', '==', uid));
      await deleteDocByQuery(db.collection('paymentRequests').where('userId', '==', uid));
      if (deleteAuth && auth.currentUser && auth.currentUser.uid === uid) {
        try { await auth.currentUser.delete(); } catch (e) { console.warn('auth delete failed', e); }
      }
    }

    async function delU(uid, btn) {
      if (!confirm('حذف هذا المستخدم نهائياً؟ سيتم حذف بياناته من Firestore.')) return;
      const oldTxt = btn?.textContent;
      if (btn) { btn.disabled = true; btn.textContent = '...'; }
      try {
        await deleteUserData(uid, { deleteAuth: false });
        btn.closest('tr')?.remove();
        showT('تم الحذف وحذف البيانات المرتبطة', 'suc');
      } catch (e) {
        showT('خطأ: ' + e.message, 'err');
      } finally {
        if (btn) { btn.disabled = false; btn.textContent = oldTxt || 'حذف الحساب'; }
      }
    }
    async function deleteMyAccount() {
      if (!CU) return;
      if (!confirm('هل تريد حذف حسابك نهائياً؟ سيتم حذف كل بياناتك من المنصة.')) return;
      try {
        showT('جاري حذف الحساب...', 'inf');
        const uid = CU.uid;
        await deleteUserData(uid, { deleteAuth: true });
        await auth.signOut().catch(() => {});
        CP = null; CU = null; walBal = 0;
        updNavG();
        go('home');
        showT('تم حذف الحساب نهائياً', 'suc');
      } catch (e) {
        showT('تعذر حذف الحساب: ' + (e.message || e), 'err');
      }
    }
...
const COUNTRY_PHONE_DATA = [
  ...
  { name: 'الإمارات العربية المتحدة', dial: '+971' },
  ...
  { name: 'ليبيا', dial: '+218' },
  { name: 'تركيا', dial: '+90' },
  { name: 'المملكة المتحدة', dial: '+44' },
  { name: 'الولايات المتحدة', dial: '+1' },
  { name: 'كندا', dial: '+1' },
  { name: 'فرنسا', dial: '+33' },
  { name: 'ألمانيا', dial: '+49' },
  { name: 'الهند', dial: '+91' },
  { name: 'باكستان', dial: '+92' }
];

function buildCountryOptions() {
  return COUNTRY_PHONE_DATA.map(c => `<option value="${escapeHTML(c.name)}">${escapeHTML(c.name)} (${c.dial})</option>`).join('');
}
function buildDialOptions() {
  return COUNTRY_PHONE_DATA.map(c => `<option value="${escapeHTML(c.dial)}">${escapeHTML(c.dial)} — ${escapeHTML(c.name)}</option>`).join('');
}
function syncPhoneCountry(countryId, codeId) {
  const cSel = document.getElementById(countryId);
  const codeSel = document.getElementById(codeId);
  if (!cSel || !codeSel) return;
  const found = COUNTRY_PHONE_DATA.find(c => c.name === cSel.value) || COUNTRY_PHONE_DATA[0];
  if (found) {
    codeSel.value = found.dial;
    const ph = document.getElementById('r2Ph');
    if (ph) ph.placeholder = `مثال: ${found.dial} 1xxxxxxxxx`;
  }
}
function syncCountryFromCode(codeId, countryId) {
  const codeSel = document.getElementById(codeId);
  const cSel = document.getElementById(countryId);
  if (!cSel || !codeSel) return;
  const found = COUNTRY_PHONE_DATA.find(c => c.dial === codeSel.value) || COUNTRY_PHONE_DATA[0];
  if (found) cSel.value = found.name;
}
function initCountrySelectors() {
  const cSel = document.getElementById('r2Country');
  const codeSel = document.getElementById('r2Code');
  if (!cSel || !codeSel) return;
  if (!cSel.dataset.ready) {
    cSel.innerHTML = '<option value="">اختر الدولة</option>' + buildCountryOptions();
    cSel.dataset.ready = '1';
  }
  if (!codeSel.dataset.ready) {
    codeSel.innerHTML = buildDialOptions();
    codeSel.dataset.ready = '1';
  }
  if (!cSel.value) cSel.value = COUNTRY_PHONE_DATA[0].name;
  syncPhoneCountry('r2Country', 'r2Code');
  cSel.onchange = () => syncPhoneCountry('r2Country', 'r2Code');
  codeSel.onchange = () => syncCountryFromCode('r2Code', 'r2Country');
}
...
function getCropStage() {
  return document.getElementById('cropImage')?.parentElement || null;
}
function cropTransform() {
  const img = document.getElementById('cropImage');
  if (!img || !cropState.ready) return;
  const totalScale = cropState.baseScale * cropState.scale;
  img.style.transform = `translate(-50%, -50%) translate(${cropState.x}px, ${cropState.y}px) scale(${totalScale}) rotate(${cropState.rotate}deg)`;
}
function fitCropImage() {
  ...
  const stageSize = Math.min(rect.width, rect.height) || 360;
  cropState.stageSize = stageSize;
  cropState.baseScale = Math.max(stageSize / img.naturalWidth, stageSize / img.naturalHeight);
  cropState.scale = 1;
  cropState.rotate = 0;
  cropState.x = 0;
  cropState.y = 0;
...
function bindCropDrag() {
  ...
  window.addEventListener('pointermove', (ev) => {
    if (!cropState.dragging || !cropState.ready) return;
    ev.preventDefault();
    cropState.x = cropState.originX + (ev.clientX - cropState.startX);
    cropState.y = cropState.originY + (ev.clientY - cropState.startY);
    cropTransform();
  });
  window.addEventListener('pointerup', () => { cropState.dragging = false; });
  window.addEventListener('pointercancel', () => { cropState.dragging = false; });
}
function openCropModal(dataUrl, mode) {
  ...
  const zoom = document.getElementById('cropZoomRange');
  if (!image) return;
  image.onload = () => fitCropImage();
  image.src = dataUrl;
  if (zoom) zoom.value = 1;
  openM('cropMod');
  bindCropDrag();
  setTimeout(() => { try { fitCropImage(); } catch (e) {} }, 80);
}
...
function setCropZoom(value) {
  ...
  if (isStep) {
    const next = Math.min(3, Math.max(0.8, cropState.scale + v));
    cropState.scale = next;
    const range = document.getElementById('cropZoomRange');
    if (range) range.value = next.toFixed(2);
  } else {
    cropState.scale = Math.min(3, Math.max(0.8, v));
  }
  cropTransform();
}
function applyCrop() {
  ...
  if (!img || !cropState.ready) return;
  const stage = getCropStage();
  const rect = stage?.getBoundingClientRect?.() || { width: 360, height: 360 };
  const stageSize = Math.min(rect.width, rect.height) || cropState.stageSize || 360;
  const outSize = 900;
  ...
  ctx.save();
  ctx.translate(outSize / 2 + (cropState.x * ratio), outSize / 2 + (cropState.y * ratio));
  ctx.rotate((cropState.rotate || 0) * Math.PI / 180);
...
window.openM = function(id) {
  ...
  const out = _skillakOpenM(id);
  if (id === 'regMod') setTimeout(initCountrySelectors, 30);
  return out;
};
...
window.openRegAs = function(role) {
  ...
  const out = _skillakOpenRegAs(role);
  setTimeout(initCountrySelectors, 30);
  return out;
};
...
window.go = function(name) {
  ...
  const out = _skillakGo(name);
  if (name === 'wallet' && CP) {
    const role = CP.role || 'learner';
    const topCard = document.getElementById('topupCard');
    const wCard = document.getElementById('withdrawCard');
    if (topCard) topCard.style.display = role === 'tutor' ? 'none' : 'block';
    if (wCard) wCard.style.display = role === 'learner' ? 'none' : 'block';
  }
  return out;
};
...
window.doGoogleLogin = async function() {
  showT('تم إيقاف تسجيل Google في هذه النسخة.', 'inf');
};
...
async function buildAdminReport(uid) {
  ...
  try {
    ...
    const roleKey = u.role || 'learner';
    const isTutor = roleKey === 'tutor' || roleKey === 'both' || roleKey === 'admin';
    const isLearner = roleKey === 'learner' || roleKey === 'both' || roleKey === 'admin';
    const bookings = [...(b1.docs || []), ...(b2.docs || [])].map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
    const txs = (txSnap.docs || []).map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
    ...
    const bookingSpend = txs.filter(t => String(t.kind || '').toLowerCase() === 'booking' && String(t.type || '').toLowerCase() === 'debit').reduce((s, t) => s + Number(t.amount || 0), 0);
    const bookingEarn = txs.filter(t => String(t.kind || '').toLowerCase() === 'booking' && String(t.type || '').toLowerCase() === 'credit').reduce((s, t) => s + Number(t.amount || 0), 0);
    const totalTopup = payments.reduce((s, x) => s + Number(x.amount || 0), 0);
    const totalWithdraw = withdrawals.reduce((s, x) => s + Number(x.amount || 0), 0);
    ...
    const filename = `Skillak-${(u.name || u.email || 'report').replace(/[\\/:*?"<>|]+/g, '_').replace(/\s+/g, '_')}`;
    const walletSummary = isTutor && !isLearner
      ? `<div class="rm"><span>إجمالي الأرباح</span><strong>${bookingEarn.toFixed(2)} ج.م</strong></div><div class="rm"><span>إجمالي السحب</span><strong>${totalWithdraw.toFixed(2)} ج.م</strong></div>`
      : isLearner && !isTutor
        ? `<div class="rm"><span>إجمالي الشحن</span><strong>${totalTopup.toFixed(2)} ج.م</strong></div><div class="rm"><span>إجمالي الإنفاق</span><strong>${bookingSpend.toFixed(2)} ج.م</strong></div>`
        : `<div class="rm"><span>إجمالي الداخل</span><strong>${(totalTopup + bookingEarn).toFixed(2)} ج.م</strong></div><div class="rm"><span>إجمالي الخارج</span><strong>${(bookingSpend + totalWithdraw).toFixed(2)} ج.م</strong></div>`;
    const finCards = isTutor && !isLearner
      ? `<div class="ad-card"><strong>الأرباح</strong><div class="num">${bookingEarn.toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">جلسات معتمدة من الإدارة</div></div><div class="ad-card"><strong>السحوبات</strong><div class="num">${withdrawals.length}</div><div style="font-size:.8rem;color:var(--muted)">طلبات السحب</div></div><div class="ad-card"><strong>المحفظة</strong><div class="num">${balance.toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">الرصيد الحالي</div></div><div class="ad-card"><strong>الجلسات</strong><div class="num">${completed}</div><div style="font-size:.8rem;color:var(--muted)">مكتملة</div></div>`
      : isLearner && !isTutor
        ? `<div class="ad-card"><strong>الشحن</strong><div class="num">${totalTopup.toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">طلبات الشحن</div></div><div class="ad-card"><strong>الإنفاق</strong><div class="num">${bookingSpend.toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">الحجوزات</div></div><div class="ad-card"><strong>المحفظة</strong><div class="num">${balance.toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">الرصيد الحالي</div></div><div class="ad-card"><strong>الجلسات</strong><div class="num">${completed}</div><div style="font-size:.8rem;color:var(--muted)">مكتملة</div></div>`
        : `<div class="ad-card"><strong>الشحن/الأرباح</strong><div class="num">${(totalTopup + bookingEarn).toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">إجمالي الداخل</div></div><div class="ad-card"><strong>الإنفاق/السحب</strong><div class="num">${(bookingSpend + totalWithdraw).toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">إجمالي الخارج</div></div><div class="ad-card"><strong>المحفظة</strong><div class="num">${balance.toFixed(0)}</div><div style="font-size:.8rem;color:var(--muted)">الرصيد الحالي</div></div><div class="ad-card"><strong>الجلسات</strong><div class="num">${completed}</div><div style="font-size:.8rem;color:var(--muted)">مكتملة</div></div>`;
    const detailsTable = bookings.length ? bookings.map(b => `<tr><td>${b.studentId === uid ? 'حجز كطالب' : 'جلسة كمعلم'}</td><td>${b.date || '—'} ${b.timeLbl || b.time || ''}</td><td><span class="pill ${b.status === 'completed' ? 'pc' : b.status === 'refunded' ? 'pco' : b.status === 'cancelled' ? 'pca' : 'pp'}">${b.status || '—'}</span></td><td>${Number(b.total || b.price || 0).toFixed(2)} ج.م</td><td>${b.studentId === uid ? (b.tutorName || '—') : (b.studentName || '—')}</td></tr>`).join('') : '<tr><td colspan="5" style="text-align:center;padding:20px;color:var(--muted)">لا توجد جلسات بعد</td></tr>';
    const txRows = txs.length ? txs.map(t => `<tr><td>${t.kind || t.type || '—'}</td><td>${t.description || '—'}</td><td>${Number(t.amount || 0).toFixed(2)} ج.م</td><td>${t.status || '—'}</td><td>${t.createdAt?.toDate ? t.createdAt.toDate().toLocaleString('ar-EG') : '—'}</td></tr>`).join('') : '<tr><td colspan="5" style="text-align:center;padding:20px;color:var(--muted)">لا توجد معاملات بعد</td></tr>';
    card.innerHTML = `
      <div class="report-title">
        <div>
          <span class="sl" style="margin-bottom:6px">التقرير</span>
          <h3 style="margin:0;font-family:'Fraunces',serif;font-size:1.6rem">${u.name || '—'}</h3>
          <div class="report-sub">${role} · ${u.email || '—'} · آخر معاملة: ${lastTx}</div>
        </div>
        <div class="pill ${isTutor ? 'pc' : 'pp'}">${role}</div>
      </div>
      <div class="report-meta" style="margin-bottom:14px">
        ...
        <div class="rm"><span>الهاتف</span><strong>${u.phone || '—'}</strong></div>
      ...
      <div class="ad-grid" style="margin-bottom:14px">${finCards}</div>
      <div class="report-meta" style="margin-bottom:14px">${walletSummary}<div class="rm"><span>الجلسات المكتملة</span><strong>${completed}</strong></div><div class="rm"><span>المعلقة</span><strong>${pending}</strong></div></div>
      <div class="card" style="border-radius:18px;overflow:hidden;margin-bottom:14px">
        ...
        <div class="cb" style="padding:0;overflow:auto;max-height:300px">
          <table class="dtbl" style="min-width:760px"><thead><tr><th>النوع</th><th>التاريخ</th><th>الحالة</th><th>المبلغ</th><th>المعلم/الطالب</th></tr></thead><tbody>${detailsTable}</tbody></table>
        </div>
      </div>
      <div class="card" style="border-radius:18px;overflow:hidden">
        ...
        <div class="cb" style="padding:0;overflow:auto;max-height:280px">
          <table class="dtbl" style="min-width:760px"><thead><tr><th>النوع</th><th>الوصف</th><th>المبلغ</th><th>الحالة</th><th>التاريخ</th></tr></thead><tbody>${txRows}</tbody></table>
        </div>
      </div>`;
  } catch (e) {
...
window.downloadAdminReportPdf = async function() {
  ...
  const name = sel?.selectedOptions?.[0]?.textContent || 'report';
  try {
